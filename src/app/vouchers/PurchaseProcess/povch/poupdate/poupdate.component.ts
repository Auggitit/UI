...
//#endregion
@Component({
  selector: 'app-poupdate',
  templateUrl: './poupdate.component.html',
  styleUrls: ['./poupdate.component.scss'],
})
export class PoupdateComponent implements OnInit {
  ...
  loadLedger() {
    return new Promise((resolve) => {
      ...
      resolve({ action: 'success' });
      console.log("AFLOAD LED",this.gstlistData.filteredData)  
    });    
  }
  //#endregion
  //#region loadgrn
    ...
    return new Promise((resolve) => {    
        this.poapi.deleteCusFields(this.pono, this.vchType,this._branch,this._fy).subscribe(res=>{
          this.poapi.Delete_PODetails(this.pono, this.vchType,this._branch,this._fy).subscribe(res=>{
            ...
            this.poapi.deteleAllOtherLedger(this.pono, this.vchType,this._branch,this._fy).subscribe(res=>{         
                    resolve({ action: 'success' });
                  });
                  });
              });
