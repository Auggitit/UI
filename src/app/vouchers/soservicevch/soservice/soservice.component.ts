...
export class SoserviceComponent implements OnInit {
  ...
  addOtherAccDataRow(acode: any, aname: any, avalue: any, akey: any) {
    ...
    if (this.otheracclistData.filteredData)
      var obj = this.otheracclistData.filteredData.find(
        (o) => o.acckey === akey
      );
    console.log(obj);
    if (obj == undefined || obj.length == 0) {
  ...
  getMaxPoNo(type: any) {
    return new Promise((resolve) => {
      this.soapi
        .getMaxInvoiceNo(type, this._branch, this._fy, this._fyname)
        .subscribe((res) => {
          this.pono = res;
          resolve({ action: 'success' });
        });
  ...
  gotolist() {
    this.router.navigateByUrl('service-sales-order-list');
  }
}
