...
export class SalesupdateComponent implements OnInit {
  ...
  loadSO() {
    return new Promise((resolve) => {
      this.salesapi.get_Sales(this.invno.toString()).subscribe((res) => {
        console.log('sale load', res);
        this.invdate = res[0].invdate;
        ...
        this.findvendor(this.vendorcode).then((ok) => {
  ...
  insertGrn() {
    ...
    return new Promise((resolve) => {
      var postData = {
        ...
        branch: this.branch,
        fy: this.fy,
        tcsRate: parseFloat(this.tcsrate).toFixed(2),
        tcsValue: parseFloat(this.tcsvalue).toFixed(2),
        closingValue: parseFloat(this.closingtotal).toFixed(2),
  ...
  deleteExistingGRN() {
    return new Promise((resolve) => {
      this.salesapi
        .Delete_Sales(this.invno, this.vchtype, this.branch, this.fy)
        .subscribe((res) => {
          this.salesapi
            .Delete_SalesDetails(this.invno, this.vchtype, this.branch, this.fy)
            .subscribe((res) => {
              this.salesapi
                .Delete_Accounts(this.invno, this.vchtype, this.branch, this.fy)
                .subscribe((res) => {
                  this.salesapi
