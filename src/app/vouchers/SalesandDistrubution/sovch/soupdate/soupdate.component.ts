...
@Component({
  selector: 'app-soupdate',
  templateUrl: './soupdate.component.html',
  styleUrls: ['./soupdate.component.scss'],
  providers: [SalesArray, SalescusComponent],
})
export class SoupdateComponent implements OnInit {
            ...
            this.loadSavedAccounts().then(()=>{
              this.loadDefalutAccounts().then(()=>{
                this.loadgst().then(()=>{
                 ...
                 var _sono = this.activatedRoute.snapshot.paramMap.get('id');  
                 if(_sono !== null)
                 {
                   this.sono = decodeURIComponent(_sono);
                 }  
  ...
  {
    return new Promise((resolve) => { 
      this.soapi.get_SO(this.sono).subscribe((res)=>{  
        console.log("po",res);        
        this.sono = res[0].sono;
        this.sodate = res[0].sodate;
        this.ssid = res[0].soid;
  ...
  loadSODetails()
  {
    return new Promise((resolve) => { 
      this.emprtyGrid();
      this.soapi.get_SODetails(this.sono).subscribe(res=>{          
        console.log(res);  
        res.forEach((v:any) => {                      
          this.addDataRow(
            v.productcode,
  ...
  sdefload: any;
  loadSavedSdef() {
    this.salesapi.getSavedDefSalesFields(this.sono).subscribe((res) => {
  ...
  //     console.log(this.subtotal,"s2");
  //     console.log(this.subwithoutcharges,"s3");      
  ...
  {
    return new Promise((resolve) => { 
      this.salesapi.Delete_SO(this.sono,this.vchtype,this.branch,this.fy).subscribe(res=>{
        this.salesapi.Delete_SODetails(this.sono,this.vchtype,this.branch,this.fy).subscribe(res=>{
          this.salesapi.Delete_SOCusFields(this.sono,this.vchtype,this.branch,this.fy).subscribe(res=>{
          this.salesapi.deteleAllOtherLedger(this.sono,this.vchtype,this.branch,this.fy).subscribe(res=>{   
              resolve({ action: 'success' });
            }); 
            }); 
         });                  
