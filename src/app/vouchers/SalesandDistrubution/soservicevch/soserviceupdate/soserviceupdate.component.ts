...
@Component({
  selector: 'app-soserviceupdate',
  templateUrl: './soserviceupdate.component.html',
  styleUrls: ['./soserviceupdate.component.scss'],
  providers: [SalesArray, SalescusComponent],
})
export class SoserviceupdateComponent implements OnInit {
  ...
  ngOnInit(): void {  
    ...
    var pono = this.activatedRoute.snapshot.paramMap.get('id');  
   if(pono !== null)
   {
     this.sono = decodeURIComponent(pono);
   }  
  ...
  loadLedger() {
    return new Promise((resolve) => {
      this.emptyGstGrid();
      this.soapi.get_Ledger(this.sono.toString()).subscribe((res) => {
        console.log(res,"add ledger");
  ...
  {
    return new Promise((resolve) => { 
      this.soapi.get_SSO(this.sono).subscribe((res)=>{  
        console.log("po",res);        
        this.sono = res[0].sono;
        this.ssid = res[0].ssoid;
        this.sodate = res[0].sodate;
  ...
  sdefload: any;
  loadSavedSdef() {
    this.salesapi.getSavedDefSales(this.sono).subscribe((res) => {
  ...
  loadSODetails()
  {
    return new Promise((resolve) => { 
      this.emprtyGrid();
      this.soapi.get_SSODetails(this.sono).subscribe(res=>{          
        console.log(res);  
        res.forEach((v:any) => {                      
          this.addDataRow(
            v.productcode,
  ...
  goback()
{
  ...
  deleteExistingSO()
  {
    return new Promise((resolve) => { 
      this.soapi
          .Delete_SO(this.sono, this.vchtype, this.branch, this.fy)
          .subscribe((res) => {
            this.soapi
              .Delete_SODetails(this.sono, this.vchtype, this.branch, this.fy)
              .subscribe((res) => {
                this.soapi
                  .Delete_SOCusFields(this.sono, this.vchtype, this.branch, this.fy)
                  .subscribe((res) => {
                    this.soapi
                      .deteleAllOtherLedger(this.sono, this.vchtype, this.branch, this.fy)
                      .subscribe((res) => {   
              resolve({ action: 'success' });
            }); 
            }); 
  ...
  insertGrn() {
    return new Promise((resolve) => {
      var postData = {
        ...
        discountTotal: parseFloat(this.disctotal).toFixed(2),
        cgstTotal: parseFloat(this.cgsttotal).toFixed(2),
        sgstTotal: parseFloat(this.sgsttotal).toFixed(2),
        igstTotal: parseFloat(this.igsttotal).toFixed(2),
        //tds: parseFloat(this.tdstotal).toFixed(2),
        roundedoff: parseFloat(this.roundedoff).toFixed(2),
        net: parseFloat(this.nettotal).toFixed(2),       
        rCreatedDateTime : new Date,
        rStatus: 'A',      
  ...
  insertCustomFields() {
    return new Promise((resolve) => {
      ...
      this.soapi.insertCusFields(this.cusFields).subscribe((res) => {
        resolve({ action: 'success' });
      });
