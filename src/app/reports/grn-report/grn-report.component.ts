  ...
  downloadAsPDF() {
    if (this.form.value.SelectSaveOptions === 0) {
      ...
      html2canvas(data, { scale: 2 }).then((canvas) => {
        ...
        pdf.text(
          ' Goods Receipt Note (GRN) Summary(' + timeDuration + ')',
          200,
          50
        );
        ...
        pdf.setLineWidth(2);
        pdf.text('Recent Goods Receipt Note (GRN)', 240, (topValue += 50));
        pdf.setFontSize(12);
      ...
      ws['!cols'] = wsCols;
      XLSX.utils.sheet_add_aoa(ws, [['Goods Receipt Note (GRN) Summary']], {
        origin: 'E1',
      });
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Goods Receipt Note (GRN) Summary');
