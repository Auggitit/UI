...
export class LedgerlistComponent implements OnInit {
  ...
  search(): void {
    if (this.searchledger.length > 0) {
      this.ventorTemp = [];
      this.ventorTemp.push(
        this.vendorData.filter((i: { companyDisplayName: string }) =>
          i.companyDisplayName
            .toLowerCase()
            .includes(this.searchledger.toLowerCase())
        )
      );
  ...
  editled(l: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.router.navigateByUrl('ledgerupdate/' + l.id);
      }
    });
  }
