...
export class CashReceiptListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadData();
    this.CashReceiptForm.valueChanges.subscribe((values) =>
      this.loadData(values)
    );
  }

  onClickButton(): void {
    this.router.navigateByUrl('cash-receipt-create');
  }
...
// }  
  downloadAsPDF() {
    console.log(
      'this.CashReceiptForm.value.SelectSaveOptions',
      this.CashReceiptForm.value.SelectSaveOptions
    );
    if (this.CashReceiptForm.value.SelectSaveOptions === 0 || this.CashReceiptForm.value.SelectSaveOptions === 'PDF') {
      let topValue = 0;
      ...
      html2canvas(data, { scale: 2 }).then(() => {
        ...
        pdf.setLineWidth(2);
        pdf.text('CashReceipt Summary', 240, (topValue += 50));
        pdf.setFontSize(12);
      ...
      let element = document.getElementById('CashReceiptTable')!;

      const wb: XLSX.WorkBook = XLSX.utils.book_new();
      wb.Props = {
        Title: 'CashReceipt List',
      };
      var ws: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet([['']]);
      var wsCols = [{ wch: 7 }, { wch: 15 }, { wch: 45 }, { wch: 45 }];
      ws['!cols'] = wsCols;
      XLSX.utils.sheet_add_aoa(ws, [['CashReceipt Summary']], { origin: 'E1' });
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'CashReceipt Summary');
      XLSX.writeFile(wb, 'cashReceipt Report.xlsx');
    }
  }
