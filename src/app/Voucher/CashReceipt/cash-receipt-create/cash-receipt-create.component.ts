...
@Component({
  ...
  templateUrl: './cash-receipt-create.component.html',
  styleUrls: ['./cash-receipt-create.component.scss']
})
export class CashReceiptCreateComponent implements OnInit {
  cashReceiptForm!: FormGroup;
  oldid: any;
  vchType = 'Cash Receipt';
  vchdate: any;
  chqdate: any;
  refdate: any;
  ...
  isCreateCashReceipt: boolean = true;
  ...
  ngOnInit(): void {
    ...
    if (this.activatedRoute.snapshot.queryParams['type'] == 'edit') {
      this.isCreateCashReceipt = false;
      if (this.oldid !== null) {
    ...
    return new Promise((resolve) => {
      this.receiptapi.getVoucherListForEdit(vchno, vchtype).subscribe((res) => {
        if (JSON.parse(res).length > 0) {
          ...
          this.cashReceiptForm.patchValue({
            cvoucherno: vchno,
  ...
  loadPendingInvoices(ledgercode: any) {
    ...
    const dialogRef = this.dialog.open(ListingTableDialogComponent, {
      data: {
        ...
        _fy: this._fy,
        isCreate: this.isCreateCashReceipt,
        vchno: this.vchno,
  ...
  onClickButton(): void {
    this.router.navigateByUrl('cash-receipt-list');
  }
      ...
      var postdata = [
        {
          ...
          vchno: this.cashReceiptForm.get('cvoucherno')?.value?.toString(),
          vchdate: new Date(this.cashReceiptForm.get('cvchdate')?.value),
          vchtype: this.vchType,
          entrytype: 'DR',
          cr: 0,
          dr: this.cashReceiptForm.get('cvoucheramount')?.value,
          ...
          gst: '',
          hsn: '',
          remarks: this.cashReceiptForm.get('cremarks')?.value,
        },
      ];
      ...
      var postdata = [
        {
          ...
          vchtype: this.vchType,
          entrytype: 'CR',
          cr: this.cashReceiptForm.get('cvoucheramount')?.value,
          dr: 0,
          rCreatedDateTime: new Date(),
          rStatus: 'A',
          comp: this._company,
  ...
  insertVoucher() {
    console.log(
      ...
      this.cashReceiptForm.get('cvoucherno')?.value,
      this.drAccountCode,
      this.crAccountCode
    );
    ...
    return new Promise((resolve) => {
      var postdata = {
        ...
        acccode: this.crAccountCode?.toString(),
        vchtype: this.vchType,
        paymode: this.cashReceiptForm.get('cpaymentmode')?.value,
        chqno: this.cashReceiptForm.get('cchequeno')?.value,
        chqdate: new Date(this.cashReceiptForm.get('cchequedate')?.value),
        refno: this.cashReceiptForm.get('crefno')?.value,
        refdate: new Date(this.cashReceiptForm.get('crefdate')?.value),
        amount: this.cashReceiptForm.get('cvoucheramount')?.value,
        rCreatedDateTime: new Date(),
        rStatus: 'A',
        remarks: this.cashReceiptForm.get('cremarks')?.value,
      };
  ...
  insertPayment(
    ...
    return new Promise((resolve) => {
      var postdata = {
        ...
        rStatus: 'A',
        remarks: this.cashReceiptForm.get('cremarks')?.value,
        entryno: this.cashReceiptForm.get('cvoucherno')?.value?.toString(),
      };
  ...
  validate() {
    ...
    if (this.drAccountCode == '' || this.drAccountCode == undefined) {
      ...
      this.cashReceiptForm.get('cpaymentmode')?.value == '' ||
      this.cashReceiptForm.get('cpaymentmode')?.value == undefined
    ) {
      Swal.fire({
        icon: 'error',
        title: 'Please Select!',
      ...
      this.cashReceiptForm.get('cpaymentmode')?.value == 'CHEQUE' &&
      (this.cashReceiptForm.get('cchequeno')?.value == undefined ||
        this.cashReceiptForm.get('cchequeno')?.value == '' ||
        this.cashReceiptForm.get('cchequedate')?.value == undefined ||
        this.cashReceiptForm.get('cchequedate')?.value == '')
    ) {
      Swal.fire({
        icon: 'error',
        title: 'Please Fill!',
      ...
      this.cashReceiptForm.get('cpaymentmode')?.value == 'ONLINE' &&
      (this.cashReceiptForm.get('crefno')?.value == undefined ||
        this.cashReceiptForm.get('crefno')?.value == '' ||
        this.cashReceiptForm.get('crefdate')?.value == undefined ||
        this.cashReceiptForm.get('crefdate')?.value == '')
    ) {
      Swal.fire({
        icon: 'error',
        title: 'Please Fill!',
      ...
      this.cashReceiptForm.get('cvoucheramount')?.value == '' ||
      this.cashReceiptForm.get('cvoucheramount')?.value == undefined
    ) {
      Swal.fire({
        icon: 'error',
        title: 'Please Fill!',
