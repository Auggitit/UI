...
export class BankPaymentListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadData();
    this.BankPaymentForm.valueChanges.subscribe((values) =>
  ...
  downloadAsPDF() {
    console.log(
      'this.BankPaymentForm.value.SelectSaveOptions',
      this.BankPaymentForm.value.SelectSaveOptions
    );
    if (
      this.BankPaymentForm.value.SelectSaveOptions === 0 ||
      this.BankPaymentForm.value.SelectSaveOptions === 'PDF'
    ) {
      let topValue = 0;
      ...
      html2canvas(data, { scale: 2 }).then(() => {
        ...
        pdf.setLineWidth(2);
        pdf.text('BankPayment Summary', 240, (topValue += 50));
        pdf.setFontSize(12);
      ...
      let element = document.getElementById('bankPaymentTable')!;
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Bank Payment Summary');
      XLSX.writeFile(wb, 'BankPayment Report.xlsx');
    }
  }
}
